...
def addexpense():
    ...
    p2 = date[11:13]
    p3 = date[14:]
    p4 = p1 + " " + p2 + ":" + p3 + ":00" #'2002-10-20 12:00:00'
    print(p4)
    print(session)
    # cursor = mysql.connection.cursor()
...
@app.route('/delete/<string:id>/<string:name>', methods = ['POST', 'GET' ])
def delete(id,name):
    #  cursor = mysql.connection.cursor()
    #  cursor.execute('DELETE FROM expenses WHERE  id = {0}'.format(id))
    #  mysql.connection.commit()
    param = "DELETE FROM expenses WHERE userid = " + "\'" + id + "\'" + " and expensename = " + "\'" + name + "\'"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    print(res)
...
@app.route('/edit/<id>/<name>', methods = ['POST', 'GET' ])
def edit(id,name):
    # cursor = mysql.connection.cursor()
    # cursor.execute('SELECT * FROM expenses WHERE  id = %s', (id,))
    # row = cursor.fetchall()
    param = "SELECT * FROM expenses WHERE userid = " + "\'" + id + "\'" + " and expensename = " + "\'" + name + "\'"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    row = []
    while dictionary != False:
...
def update(id,name):
  if request.method == 'POST' :
    ...
    #   mysql.connection.commit()
      ...
      sql = "UPDATE expenses SET date = ? , expensename = ? , amount = ?, paymode = ?, category = ? WHERE userid = ? and expensename = ?"  
      stmt = ibm_db.prepare(ibm_db_conn, sql)
...
def limitnum():
     if request.method == "POST":
        ...
        #  mysql.connection.commit()
         param1 = "drop table limits"
         res = ibm_db.exec_immediate(ibm_db_conn, param1)
         param2 = "create table limits(userid int,limitss varchar(500));"
         res = ibm_db.exec_immediate(ibm_db_conn, param2)
